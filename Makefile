...
pkgdatadir = $(datadir)/apertium-{{languageCode1}}-{{languageCode2}}
pkgincludedir = $(includedir)/apertium-{{languageCode1}}-{{languageCode2}}
pkglibdir = $(libdir)/apertium-{{languageCode1}}-{{languageCode2}}
pkglibexecdir = $(libexecdir)/apertium-{{languageCode1}}-{{languageCode2}}
...
CONFIG_CLEAN_FILES = apertium-kas.pc
...
am__installdirs = "$(DESTDIR)$(apertium_kasdir)" \
	"$(DESTDIR)$(pkgconfigdir)"
DATA = $(apertium_kas_DATA) $(noinst_DATA) $(pkgconfig_DATA)
am__tagged_files = $(HEADERS) $(SOURCES) $(TAGS_FILES) $(LISP)
am__DIST_COMMON = $(srcdir)/Makefile.in $(srcdir)/apertium-kas.pc.in \
...
PACKAGE_STRING = Apertium {{languageName1}}-{{languageName2}} 0.1.0
PACKAGE_TARNAME = apertium-{{languageCode1}}-{{languageCode2}}
...
apertium_kas_DATA = $(TARGETS_COMMON) $(LANG1).prob
...
$(am__aclocal_m4_deps):
apertium-kas.pc: $(top_builddir)/config.status $(srcdir)/apertium-kas.pc.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
install-apertium_kasDATA: $(apertium_kas_DATA)
	@$(NORMAL_INSTALL)
	@list='$(apertium_kas_DATA)'; test -n "$(apertium_kasdir)" || list=; \
	if test -n "$$list"; then \
	  echo " $(MKDIR_P) '$(DESTDIR)$(apertium_kasdir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(apertium_kasdir)" || exit 1; \
	fi; \
	for p in $$list; do \
	...
	while read files; do \
	  echo " $(INSTALL_DATA) $$files '$(DESTDIR)$(apertium_kasdir)'"; \
	  $(INSTALL_DATA) $$files "$(DESTDIR)$(apertium_kasdir)" || exit $$?; \
	done

uninstall-apertium_kasDATA:
	@$(NORMAL_UNINSTALL)
	@list='$(apertium_kas_DATA)'; test -n "$(apertium_kasdir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	dir='$(DESTDIR)$(apertium_kasdir)'; $(am__uninstall_files_from_dir)
...
installdirs:
	for dir in "$(DESTDIR)$(apertium_kasdir)" "$(DESTDIR)$(pkgconfigdir)"; do \
...
install-data-am: install-apertium_kasDATA install-data-local \
...
uninstall-am: uninstall-apertium_kasDATA uninstall-pkgconfigDATA
...
.PHONY: all all-am am--refresh check check-am clean clean-generic \
	...
	install install-am install-apertium_kasDATA install-data \
	...
	uninstall-am uninstall-apertium_kasDATA \
	uninstall-pkgconfigDATA
...
install-data-local:
	...
	$(INSTALL_DATA) $(BASENAME).post-$(LANG1).dix $(DESTDIR)$(apertium_kas_srcdir)
clean-local:
	-rm -rf .deps modes
...
test: all
	echo "TODO test sentence" | apertium -d . kas-morph | tee .test-mt
